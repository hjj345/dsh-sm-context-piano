...
插件在聊天内容左侧显示一条固定高度的紧凑横线轨道。琴键只代表用户消息和模型可见文本输出：连续模型输出合并，遇到工具、编辑、推理或其他非输出内容后重新分段。轨道最多显示当前节点前后的 20 根琴键，点击边界琴键即可继续浏览更早或更晚的节点。
...
- **固定琴键窗口**：固定 346px 高度、18px 间距、最多 20 根，不为容纳全部历史而压缩。

...
语言切换只作用于插件设置页，立即生效且重启后保留；DSH 一级导航名称和琴键轨道无障碍名称仍跟随 DSH 全局语言。轨道总高度自动按 `(最大数量 - 1) × 间距 + 琴键高度` 计算，例如默认值为 `(20 - 1) × 12 + 2 = 230px`。设置页提供恢复默认值操作，并显示版本 `v1.0`、发布日期、作者、邮箱、安装命令及待公布的 GitHub 状态。

...
插件在聊天内容左侧显示一条紧凑的横线轨道。默认横线对应用户可感知的语义主节点；同一回合的助手步骤和工具链折叠在主节点中，鼠标靠近时才动态展开。移动鼠标可连续预览各个子节点，点击即可平滑跳转到对应消息。
...
- **语义精简**：同回合助手步骤合并，工具调用和结果归入所属回合，流式中间态和回合尾等内部状态不生成琴键。
- **动态展开**：悬停包含多个节点的主琴键时，组内助手步骤和工具节点自动展开，离开后重新收拢。
...
- **键盘操作**：方向键、Home、End、Enter、Space 和 Escape 可完成选择、展开、收拢与跳转。
...
- 默认琴键对应语义主组；有独立行锚点的工具节点在悬停主组后展开。没有独立行锚点的嵌套调用仍属于其父节点。

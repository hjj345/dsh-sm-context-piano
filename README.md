...
安装后在 DSH 设置弹窗左侧打开一级导航 **琴键导航**。该项按顺序位于官方 **Agent 预设** 下方，导航图标遵循 DSH 对第三方设置项的默认齿轮回退。
...
轨道总高度自动按 `(最大数量 - 1) × 间距 + 琴键高度` 计算。例如默认值为 `(20 - 1) × 12 + 2 = 230px`。设置页提供恢复默认值操作，并显示版本 `v1.0`、发布日期、作者、邮箱、安装命令及待公布的 GitHub 状态。

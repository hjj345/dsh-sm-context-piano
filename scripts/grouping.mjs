...
assert.deepEqual(result.map(item => item.final), [false, false, false, true, false])
assert.ok(result.every(item => typeof item.final === 'boolean'))
...
console.log('  ok  visible output segmentation, fixed window, and settings bounds (23 assertions)')
